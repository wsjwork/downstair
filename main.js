...
var main = function () {
	...
	var OrdinaryStair = T.Entity.extend({
		...
		update: function (dt) {
			...
			var result = impact(player.x - player.w / 4 + 3, player.y + player.h / 4, player.x + player.w / 4 - 3, player.y + player.h / 2, this.x - this.w / 2, this.y - this.h / 2, this.x + this.w / 2, this.y, player.x_speed, player.y_speed, 0, this.y_speed);
			this.y += this.y_speed;
			//碰撞检测
	...
	var LoadStair = T.Entity.extend({
		...
		init: function (ops) {
			this.on("added", function () {
				...
				this.parent.add(this.stairarray[this.id++]);
				this.stairarray[3] = new OrdinaryStair({x: 400, y: 1250, id: this.id});
				this.parent.add(this.stairarray[this.id++]);
			});
		},
		update: function (dt) {
		...
		function () {
			...
			_.each([
				...
				["player", {
					...
					player_run: {frames: _.range(3, 6), rate: 1 / 3},
					player_down: {frames: _.range(0, 3), rate: 1 / 4}
				}],
				["staircase", {
